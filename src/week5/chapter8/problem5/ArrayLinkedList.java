// 연결 리스트 클래스(배열 커서 버전）
package week5.chapter8.problem5;
...
public class ArrayLinkedList<E> {
    ...
    // 연습문제 Q4 : 서로 같은 노드를 찾아 가장 앞쪽의 노드만 남기고 모두 삭제하는 메서드
    public void purge(Comparator<? super E> c) {
        int ptr = head;

        while (ptr != NULL) {
            // ptr 이후부터 ptr 과 동일한지 체크하는 포인터
            int samePtr = n[ptr].next;

            while(samePtr != NULL) {
                int tempPtr;

                // 동일한 노드를 발견했을 때 삭제
                if(c.compare(n[ptr].data, n[samePtr].data) == 0) {
                    tempPtr = n[samePtr].next;
                    remove(samePtr);
                    samePtr = tempPtr;
                } else {
                    samePtr = n[samePtr].next;
                }
            }
            ptr = n[ptr].next;
        }
    ...
    // 연습문제 Q5 : 머리부터 N개 뒤의 노드 참조 반환하는 메서드
